...
- `mark_chat` — дружелюбный собеседник для обычного общения (ton-of-voice, follow-up вопросы)
- `mark_code_expert` — эксперт по коду (больше технической строгости, примеров и инструментов)
- `mark_learning_mode` — самообучение (REAP, рефлексия, сохранение инсайтов)
...
- Выбор идёт по UCB (LinUCB) + лёгкие предпочтения по контексту:
  ...
  - обычные разговоры (`domain=general|None`) → приоритет `mark_chat`, затем `mark_base`
