...
### ✅ Шаг C.5 — MCP сервер восстановлен (завершён) — 2025-09-29 21:05 CEST
- **C.5**: MCP сервер восстановлен на `mcp.markmind.ru`
- **C.5**: NGINX конфигурация для MCP восстановлена
- **C.5**: Keycloak работает на порту 8081
- **C.5**: Все сервисы доступны через домены с SSL

### Осталось / Опционально
- **C.6**: Мониторинг rate-limiting (корректировка при ложных 429)
- **C.7**: CSP перевести из report-only в enforce после тестирования
- **C.8**: Мониторинг работы системы в production режиме
