#!/usr/bin/env python3
"""
F2 Connection Pool Benchmark v2 - Улучшенная версия

Тестирует производительность connection pooling в реальных сценариях:
...
class F2ConnectionPoolBenchmarkV2:
    """
    Улучшенный benchmark для тестирования F2 Connection Pool оптимизации.

    Тестирует реальные сценарии:
    1. Множественные операции с одним клиентом
    ...
    def benchmark_repeated_operations(self) -> BenchmarkResult:
        ...
        for i in range(self.num_samples):
            ...
            try:
                ...
                logger.info(f"    Pooled (10 ops): {pooled_time:.2f}ms")
            ...
            try:
                ...
                logger.info(f"    Direct (10 ops): {direct_time:.2f}ms")
            except Exception as e:
                ...
                try:
                    if 'memory' in locals():
                        del memory
                    if 'direct_memory_manager' in locals():
        ...
        if not pooled_times or not direct_times:
            ...
            return BenchmarkResult("Repeated Operations", 0, 0, 0, 0, "10 операций подряд")
        ...
        result = BenchmarkResult(
            ...
            description="10 операций подряд с одним клиентом"
        )
    ...
    def benchmark_parallel_operations(self) -> BenchmarkResult:
        ...
        def pooled_operation(session_id: int) -> float:
            ...
            try:
                ...
                memory.get_short_term_context(f"parallel_session_{session_id}")
        ...
        # Тест параллельных pooled операций
        start_time = time.time()
        with ThreadPoolExecutor(max_workers=3) as executor:  # Уменьшаем количество потоков
            pooled_futures = [executor.submit(pooled_operation, i) for i in range(5)]  # Уменьшаем количество операций
            [future.result() for future in as_completed(pooled_futures)]
        pooled_total_time = (time.time() - start_time) * 1000

        # Тест параллельных direct операций
        start_time = time.time()
        with ThreadPoolExecutor(max_workers=3) as executor:  # Уменьшаем количество потоков
            direct_futures = [executor.submit(direct_operation, i) for i in range(5)]  # Уменьшаем количество операций
            [future.result() for future in as_completed(direct_futures)]
        ...
        result = BenchmarkResult(
            ...
            improvement_percent=improvement,
            samples=1,
            description="5 параллельных операций (3 потока)"
        )
    ...
    def run_all_benchmarks(self) -> list[BenchmarkResult]:
        ...
        logger.info("🚀 Запускаем F2 Connection Pool Benchmark v2...")
    ...
    def print_summary(self):
        ...
        logger.info("\n" + "="*70)
        logger.info("📊 F2 CONNECTION POOL BENCHMARK v2 RESULTS")
        logger.info("="*70)
...
def main():
    ...
    benchmark = F2ConnectionPoolBenchmarkV2(num_samples=5)
