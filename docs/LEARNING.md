...
Text documents from the paired owner's original private messages may be saved to the workspace, up to 512 KiB. Their caption supplies the instruction; file contents remain data. Without a caption, the default request is a brief summary. Incoming code is not executed on receipt. PNG/JPEG photos and documents use a separate private, immutable image store and accompany only their own task; visual interpretations remain model claims, not accepted facts. See [image limits and retention](IMAGES.md). Original private voice messages use the [OpenAI transcription route](VOICE.md); their derived text remains `unverified_transcription`. PDF, arbitrary audio documents and video are unsupported.
