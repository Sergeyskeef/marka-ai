...
from .fs_tools import confirm_write, fs_glob, fs_read, fs_write, security_request_write
...
@track_tool_call("fs_write")
async def fs_write_tool(path: str, content: str, mode: str = "w") -> Dict[str, Any]:
    return await fs_write(path, content, mode)


@mcp.tool(
    name="security_request_write",
    description="Create a pending write request requiring confirmation.",
)
@track_tool_call("security_request_write")
async def security_request_write_tool(path: str, content: str) -> Dict[str, Any]:
    return await security_request_write(path, content)


@mcp.tool(
    name="confirm_write", description="Approve or reject a pending write request."
)
@track_tool_call("confirm_write")
async def confirm_write_tool(request_id: str, allow: bool) -> Dict[str, Any]:
...
@track_tool_call("git_local_commit")
