...
Система памяти была усилена для повышения надежности и производительности:

### Что включено:
- **Пул соединений** - переиспользование HTTP соединений к Graphiti
- **Ретраи** - автоматические повторы при сбоях (3 попытки с экспоненциальной задержкой)
- **Валидация метаданных** - проверка через Pydantic модели
- **Опциональный кеш** - Redis кеширование результатов поиска (60 сек TTL)
- **Логирование латентности** - мониторинг времени выполнения операций

### Как включить кеш:
Добавьте в `.env`:
```
REDIS_URL=redis://:PASSWORD@marka-redis:6379/0
```

### Применение индексов Neo4j:
```bash
make neo4j-init
```

### Метрики:
Смотрите логи `mark-mcp` (уровень INFO) по латентности операций:
- `memory_search ok query=... limit=... ms=...`
- `memory_upsert ok text_len=... ms=...`
