...
class BrainProcessor:
    ...
    def process(self, message: str, chat_id: int, user_id: int | None = None, timestamp: datetime | None = None) -> dict[str, Any]:
        """
    ...
    async def _process_async(self, message: str, chat_id: int, user_id: int | None = None, timestamp: datetime | None = None) -> dict[str, Any]:
        """
    ...
    async def _enrich_with_context(self, message: str, chat_id: int, user_id: int | None, timestamp: datetime) -> dict[str, Any]:
...
НЕ показывай анализ - сразу отвечай пользователю."""
    ...
    async def _save_to_memory(self, message: str, result: ProcessingResult, chat_id: int, user_id: int | None, timestamp: datetime):
        ...
        try:
            ...
            logger.debug("💾 Взаимодействие сохранено в память")

        except Exception as e:
            logger.error(f"❌ Ошибка сохранения в память: {e}")
            raise  # Критично для архитектуры "все через мозг"

    async def _save_error_to_memory(self, message: str, error: str, chat_id: int, user_id: int | None, timestamp: datetime):
